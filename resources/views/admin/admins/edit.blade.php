{{-- Form Edit Data Administrator (Admin Portal Theme) --}}
@extends('layouts.app')

@section('title', 'Nginap Admin - Edit Akun Admin: ' . $user->name)
...
@section('content')
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-8">
            ...
            <!-- RIGHT MAIN CONTENT: FORM EDIT ADMIN -->
            <div class="lg:col-span-9 space-y-6">

                <!-- Header Title -->
                <div class="flex items-center justify-between">
                    <div>
                        <h1 class="text-3xl font-black text-stone-900 font-serif tracking-tight">Edit Data Administrator</h1>
                        <p class="text-xs text-stone-400 mt-1">Perbarui data profil akun pengelola admin {{ $user->name }} ({{ $user->email }}).</p>
                    </div>

                    <a href="{{ route('admin.admins.index') }}" class="px-4 py-2 rounded-full border border-stone-300 text-stone-700 font-bold text-xs hover:bg-stone-50 transition">
                ...
                <div class="bg-white rounded-3xl border border-stone-200/80 shadow-sm p-6 sm:p-8">
                    <form action="{{ route('admin.admins.update', $user) }}" method="POST" class="space-y-6">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            ...
                            <div>
                                <label class="block text-xs font-extrabold text-stone-900 uppercase tracking-wider mb-2">
                                    Alamat Email Admin <span class="text-rose-600">*</span>
                                </label>
                        ...
                        <div class="pt-4 border-t border-stone-100">
                            ...
                            <input type="password" name="password" placeholder="Kosongkan jika tidak ingin mengubah password admin ini" class="w-full bg-stone-50 border border-stone-200 rounded-2xl px-4 py-3 text-xs font-semibold text-stone-900 focus:outline-none focus:border-amber-700 transition">
                            <span class="text-[10px] text-stone-400 mt-1 block">Minimal 6 karakter jika ingin mereset password admin.</span>
                            @error('password')
                                <p class="text-rose-600 text-[11px] mt-1 font-semibold">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="pt-6 flex items-center justify-end gap-3 border-t border-stone-100">
                            <a href="{{ route('admin.admins.index') }}" class="px-6 py-3 rounded-full border border-stone-200 text-stone-600 font-bold text-xs hover:bg-stone-50 transition">
                                Batal
                            </a>
