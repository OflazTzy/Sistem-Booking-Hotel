{{-- Halaman Form Edit Kamar (Admin Dark Sidebar Theme) --}}
@extends('layouts.app')

@section('title', 'Edit Kamar ' . $room->room_number . ' - Nginap Admin')
...
@section('content')
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-8">
            ...
            <!-- RIGHT MAIN CONTENT: EDIT ROOM FORM -->
            <div class="lg:col-span-9 space-y-6">

                <!-- Header Title -->
                <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    <div>
                        <h1 class="text-3xl sm:text-4xl font-black text-stone-900 font-serif tracking-tight">Edit Data Kamar {{ $room->room_number }}</h1>
                        <p class="text-xs text-stone-400 mt-1">Perbarui nomor, tipe, harga, atau status ketersediaan kamar</p>
                    </div>
                ...
                <div class="bg-white rounded-3xl border border-stone-200/80 shadow-sm p-6 sm:p-8 max-w-2xl">
                    ...
                    <form action="{{ route('rooms.update', $room) }}" method="POST" class="space-y-4">
                        ...
                        <div>
                            ...
                            <input type="number" id="price" name="price" value="{{ old('price', $room->price) }}" required min="0" step="10000"
                                class="w-full px-4 py-3 bg-stone-50 border border-stone-200 rounded-2xl text-stone-800 text-xs font-semibold focus:outline-none focus:border-amber-700 transition">
                        </div>
                        ...
                        <div>
                            ...
                            <select id="status" name="status" required class="w-full px-4 py-3 bg-stone-50 border border-stone-200 rounded-2xl text-stone-800 text-xs font-semibold focus:outline-none focus:border-amber-700 transition">
                                <option value="available" {{ old('status', $room->status) == 'available' ? 'selected' : '' }}>Available (Tersedia)</option>
                                <option value="occupied" {{ old('status', $room->status) == 'occupied' ? 'selected' : '' }}>Occupied (Terisi / Terpesan)</option>
                            </select>
                        </div>
                        ...
                        <div class="pt-4 border-t border-stone-100">
                            <button type="submit" class="w-full py-3.5 px-6 rounded-full bg-[#8a6225] hover:bg-[#73501d] text-white font-extrabold text-xs shadow-lg transition flex items-center justify-center gap-2">
                                <i class="fa-solid fa-floppy-disk"></i> Simpan Perubahan Data Kamar
                            </button>
                        </div>
                    </form>
                </div>
